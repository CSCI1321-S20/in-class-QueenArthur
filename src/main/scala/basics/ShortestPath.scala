...
/**
  * Changes to make for graphics game:
  * Int to Double
  * wall check becomes isClear
  * check for start location being in a wall
  * check for isClear covers checking for out of bounds
  */
object shortestPath {
  val maze = Array(
  ...
  def breadthFirstShortestPath(sx: Int, sy: Int, ex: Int, ey: Int): Int = {
    val queue = new ArrayQueue[(Inc, Int, Int)]()
